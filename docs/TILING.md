...
| --- | --- | --- |
| `monocle.py` | Every window fills the display; switch with focus. The one to copy when starting your own. | none |
...
| `gap` | The space to leave between windows and at the display's edges, in points, resolved by mimi: `tiling.gap` when the config sets it, else the macOS tiled-window margin that `resize_window` honours, or 0 when that is off. Use it as given and tiled windows line up with hand-placed ones. |
...
# tiling.gap from the config, else the macOS tiled-window margin, else 0.
GAP = inp["gap"]
