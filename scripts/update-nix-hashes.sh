...
# and the two release zip hashes in nix/package.nix follow the newest GitHub
...
if [ "$latest" != "$current" ]; then
	...
	for asset in darwin-arm64 darwin-amd64; do
		url="https://github.com/$repo/releases/download/v$latest/mimi-$asset.zip"
		hash="sha256-$(curl -fsSL "$url" | openssl dgst -sha256 -binary | base64)"
		perl -0pi -e "s|(mimi-$asset\.zip\";\n\s*sha256 = \")[^\"]*|\${1}$hash|" "$pkg"
	done
