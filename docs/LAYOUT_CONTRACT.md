...
| `unmanaged` | array of integer | Stops watching those windows: a drag of one raises no pass and shows no drop zone, and they are handed back in the input's `unmanaged`. mimi reclassifies only the windows this input listed, so a run for one display never speaks for another's. A window stays unmanaged until a later run for the same display leaves it out. |
