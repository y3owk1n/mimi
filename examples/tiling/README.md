...
| `rules.py` | The code the others share: the area to fill, `serve()`, which reads the input and runs the layout in either mode, `write_output`, the temporary maximise that every layout except `monocle.py` answers as `togglemax`, and `min_sizes()` and `fit()`, which `strip.py` and `bsp.py` use to give a window that refuses a smaller size at least what it accepts. |
