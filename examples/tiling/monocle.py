...
names one, and with [tiling.stackbar] enabled mimi draws the others as cards
behind the focused window. That is the one thing monocle otherwise cannot tell
you, since a display with six windows on it looks like a display with one.
...
from rules import area, gap, serve, shown, unmanaged_of, write_output
...
def main(inp):
    ...
    stacks = [{"windows": numbers, "active": shown(inp, numbers)}] if numbers else []
