...
- (void)checkPowerSources {
    ...
    for (CFIndex i = 0; i < count; i++) {
        ...
        if (curCapRef) CFNumberGetValue(curCapRef, kCFNumberIntType, &curCap);
        if (maxCapRef) CFNumberGetValue(maxCapRef, kCFNumberIntType, &maxCap);
...
static OSStatus AudioPropertyListener(AudioObjectID inObjectID,
