...
| `focusKeptOut` | `true` when the focused window is one a `[[tiling.rules]]` entry with `manage = false` keeps from the layout, so `focused` is -1 while a window has focus. Absent otherwise. A layout that hands focus out after a close should leave it alone then. |
...
layout. A
drag of one with a modifier held does, which is what lets the mouse tile it
again.
