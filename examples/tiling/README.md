...
| `rules.py` | What the three share: the float rules (edit the bundle ids and title patterns here), running a layout once per display with state per display, reading the input and writing the output, and the temporary maximise every layout answers as `togglemax`. |
